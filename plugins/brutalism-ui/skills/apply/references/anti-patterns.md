...
genuinely wants blur, it is a different style — hand it to `glassmorphism-ui` for a
blur-only backdrop, or to `liquid-glass-ui` for a refracting one, and say so.
...
the plugin. All ten style languages ship one, so every row below resolves to something the
user can install; the owning doc is named beside it for the argument behind the handoff.

| Request | Actually |
|---|---|
| Soft blurred shadows, same-hue extrusion, "pressed-in" | `neumorphism-ui` — `docs/02-neumorphism.md` |
| Puffy pastel surfaces, radii ≥ 24px, inset sheen | `claymorphism-ui` — `docs/04-claymorphism.md` |
| Translucent panels, frosted chrome, `backdrop-filter` | `glassmorphism-ui` |
| Refraction, specular rims, iOS 26/27 chrome | `liquid-glass-ui` — `docs/08-liquid-glass.md` |
| Three loud layers, four typefaces, collage, blend modes | `maximalism-ui` — `docs/06-maximalism.md` |
| Strip it down to one accent and no ornament | `minimalism-ui` — `docs/05-minimalism.md`; §12 calls minimalism this style's own low-intensity fallback, so `intensity: 0` here lands in the same place |
