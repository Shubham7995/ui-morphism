...
5. **Phantom tokens, in two tiers, plus a degeneracy tripwire** — every name in the §4 token table's `Token` column must resolve to an assignment (rule 1). A name assigned nowhere in the doc is a `PHANTOM` and fails; a name §4 tables but §5 assigns at runtime is a `note` and does not fail. Separately, the check refuses to certify a table it never read: it counts table **rows** that name at least one token (rows, not unique names, so one multi-token cell cannot stand in for the column) and fails with `NONAMES` if that is under a quarter of the properties §4's CSS declares, or `NODECL` if that CSS declares none. **Read that floor as a tripwire, not as coverage.** Measured row coverage across the ten docs runs from 0.37 (doc 07, 23 rows / 62 properties) to 1.00 (doc 09, 42/42) — a spread driven by how much of a token set each doc writes as family-stem rows, which the extraction drops on purpose, so no honest coverage number can be read off it. What the floor catches is a column that names almost nothing. What it does not catch is a table that quietly stops documenting a third of its tokens: doc 07 would have to fall from 23 naming rows to 15 before it fires. A pass here means "the names the table prints are real", never "the table documents the token set".
