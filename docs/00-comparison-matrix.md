...
| **SP** | **High** — a camera, a counter-scale, a six-step ladder, and parallax that must detach under reduced motion | **Medium** — cap concurrent `backdrop-filter` surfaces at 6, composited layers at 25, `preserve-3d` nesting at 3, permanent `will-change` at 0, GPU layer memory at 96 MiB | **High** — but the conventional α = 0.72 is not the problem it was long reported to be: composited **per channel in gamma-encoded sRGB**, dark glass `#14161C` at α = 0.72 over a worst-case white backdrop resolves to ≈ `#56575C`, which measures **7.20:1** against `#FFFFFF` body text (≈ `#57595D` and **7.04:1** with the shipped `#16181E` token fill) — AAA, not the ~~3.13:1~~ the discredited luminance-averaging model reports. The real arbitrary-backdrop minimum is **α ≥ 0.62 dark glass / ≥ 0.60 light glass**, not 0.83 (doc 10 §7). SC 2.5.7 Dragging Movements is the criterion this style breaks most reliably and that nobody discusses: every `movable()`/`resizable()` panel needs a non-drag alternative. | **High** — `androidx.xr.compose` was `1.0.0-alpha16` on 2026-07-15, no stable release, breaking changes in July 2026; pin an explicit alpha or the code rots | **High** — the numbers are published (Android XR gives 0.868 dp-to-dmm, a 41° cone, a 0.75–5 m band) but the spatial reasoning is not common skill |
