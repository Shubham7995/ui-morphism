...
- **Layer count.** Each glass surface promotes to its own compositor layer, and a layer costs `width × height × 4 bytes`. On a 1170×2532 device, three full-width glass bars at the 88px toolbar height used below are 3 extra 1170×88 RGBA textures: 1170 × 88 × 4 = 411,840 bytes ≈ **0.39 MiB** each, **≈ 1.2 MiB** for the three, before the filter's intermediate buffers. Height is what makes this expensive, not count — a single full-screen 1170×2532 glass sheet is one texture of **11.3 MiB**, more than nine times all three bars together, which is why a full-bleed glass background is the one shape to refuse outright.
