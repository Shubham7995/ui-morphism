...
4. **Hard clamps are independent of intensity.** `maximalism.layerCount` ≤ 3, `liquid-glass.fillAlpha` ≥ 0.55, `neumorphism.hairlineOpacity` ≥ 0.55, `claymorphism.squishAmount` ≥ 0.93, `spatial-ui` panel alpha ≥ 0.83 on an arbitrary backdrop. Core enforces the *existence* of a clamp table per style; the values are the style's.
